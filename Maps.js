...
//In mapkeyword there is always two arguments. First one is the key value and second one is the actual value.

const rest = new Map();

//To add some element into the map, we use set method.

rest.set(("name", "Indian Cuisine"));
rest.set((1, "Jaleshwar", "Kathmandu"));
console.log(rest.set(("2", "India", "UK")));

rest.set("categories", ["Italian", "Chinese Cuisine", "Indian", "Organic"])
  .set("open", 11).set("close", 23).set(true, "We are open").set
  (false, "We are closed");


//To retrieve the element from the set we use get method.
console.log(rest.get("name"));
console.log(rest.get(true));
console.log(rest.get(1));

//Here if the time is between 11 and 23 then we print the message acoording to open and close key value 
//pair

const time = 0;
console.log(rest.get(time > rest.get('open') && rest.get(rest.get('close')));
